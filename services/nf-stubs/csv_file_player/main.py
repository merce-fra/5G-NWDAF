...
# List of endpoints to send data to
endpoints = [
    "http://gmlc:10006/data",
    "http://ran:10007/data"
]
        ...
        if field in row:
            ...
            if row[field] == '' or row[field] is None:
                ...
                try:
                    ...
                    logger.warning(f"Failed to convert {field} to {field_type.__name__}: {e}")
    ...
    async with httpx.AsyncClient() as client:
        for endpoint in endpoints:
            try:
                ...
                logger.error(f"Response body: {e.response.text}")  # Log response body (e.g., validation errors)
                logger.error(f"Request body that caused error: {data}")

            except httpx.RequestError as e:
                # Log the request error and the request body in case of network-related issues
                logger.error(f"Request error while sending to {endpoint}: {e}")
                logger.error(f"Request body that caused error: {data}")
...
if __name__ == '__main__':
    uvicorn.run(app, host='0.0.0.0', port=9999, log_level='warning', loop='asyncio')
